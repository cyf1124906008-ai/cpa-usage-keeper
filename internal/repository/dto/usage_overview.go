...
type UsageOverviewRealtimeRecord struct {
	...
	ConcurrencyLevel     []RealtimeConcurrencyPointRecord
	ErrorLevel           []RealtimeErrorLevelPointRecord
	Errors               RealtimeErrorSummaryRecord
}
...
// RealtimeConcurrencyPointRecord 是由请求开始时间与 latency 重建的并发水平。
type RealtimeConcurrencyPointRecord struct {
	Bucket             string
	AverageConcurrency float64
	PeakConcurrency    int64
}

// RealtimeErrorLevelPointRecord 是实时窗口内失败数量与失败率。
type RealtimeErrorLevelPointRecord struct {
	Bucket      string
	Failures    int64
	FailureRate float64
}

type RealtimeBreakdownItemRecord struct {
	Key   string
	Label string
	Count int64
	Share float64
...
type RealtimeErrorRecord struct {
	Timestamp  time.Time
	RequestID  string
	StatusCode int
	ErrorType  string
...
type RealtimeErrorSummaryRecord struct {
	TotalFailures  int64
	FailureRate    float64
	UnknownDetails int64
	StatusCodes    []RealtimeBreakdownItemRecord
	Types          []RealtimeBreakdownItemRecord
	Models         []RealtimeBreakdownItemRecord
	AuthFiles      []RealtimeBreakdownItemRecord
	Endpoints      []RealtimeBreakdownItemRecord
	RecentErrors   []RealtimeErrorRecord
}
