...
type RecentUsageEvent struct {
	...
	Model string
	// RequestID 和 Endpoint 用于最近错误明细，不保存请求内容。
	RequestID string
	Endpoint  string
...
func loadUsageRecentEventCacheRows(db *gorm.DB, start time.Time) ([]recentUsageEventLoadRow, error) {
	...
	if err := db.Model(&entities.UsageEvent{}).
		Select("api_group_key, provider, auth_type, model, model_alias, request_id, endpoint, timestamp, source, auth_index, failed, status_code, error_type, error_message, latency_ms, ttft_ms, input_tokens, output_tokens, reasoning_tokens, cached_tokens, cache_read_tokens, cache_creation_tokens, total_tokens").
...
func (c *UsageRecentEventCache) recentEventFromRowLocked(row recentUsageEventLoadRow) RecentUsageEvent {
	...
	return RecentUsageEvent{
		...
		RequestID:             c.pool.intern(strings.TrimSpace(row.RequestID)),
		...
		ErrorType:             c.pool.intern(strings.TrimSpace(row.ErrorType)),
		ErrorMessage:          c.pool.intern(strings.TrimSpace(row.ErrorMessage)),
