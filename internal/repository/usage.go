...
func FindUsageDataAvailableFrom(db *gorm.DB, apiGroupKey string) (*time.Time, error) {
	query := db.Model(&entities.UsageEvent{}).Select("timestamp")
	if key := strings.TrimSpace(apiGroupKey); key != "" {
		query = query.Where("api_group_key = ?", key)
	}
	var event struct {
		Timestamp time.Time
	}
	if err := query.Order("timestamp ASC").Limit(1).Take(&event).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, nil
		}
		return nil, fmt.Errorf("find earliest usage event: %w", err)
	}
	timestamp := timeutil.NormalizeStorageTime(event.Timestamp)
...
type usageOverviewRealtimeBucket struct {
	...
	concurrencyMS  int64
	intervals      []usageOverviewRealtimeInterval
}

type usageOverviewRealtimeInterval struct {
	start time.Time
	end   time.Time
}

type usageOverviewRealtimeBreakdownAccumulator struct {
	key   string
	label string
	count int64
}
...
func buildUsageOverviewRealtime(db *gorm.DB, filter dto.UsageQueryFilter, costResolver *UsageCostResolver, recentCache *UsageRecentEventCache) (dto.UsageOverviewRealtimeRecord, error) {
	...
	// 并发区间可能从可见窗口左侧开始，额外回看 5 分钟以覆盖跨边界长请求。
	eventReadStart := readStart.Add(-5 * time.Minute)
	// 所有 realtime 图表共用一次缓存读取，缓存对象不存在时再回退到 DB 窄窗查询。
	events, cacheOK := loadUsageOverviewRealtimeEventsFromRecentCache(recentCache, filter, eventReadStart, end)
	if !cacheOK {
		// 缓存对象不可用时，直接回退到 usage_events 的同窗口投影，不影响正常缓存命中语义。
		dbEvents, err := loadUsageOverviewRealtimeEventsFromDB(db, filter, eventReadStart, end)
		if err != nil {
			return dto.UsageOverviewRealtimeRecord{}, err
		}
		events = dbEvents
	...
	errorStatusCodes := map[string]*usageOverviewRealtimeBreakdownAccumulator{}
	errorTypes := map[string]*usageOverviewRealtimeBreakdownAccumulator{}
	errorModels := map[string]*usageOverviewRealtimeBreakdownAccumulator{}
	errorAuthFiles := map[string]*usageOverviewRealtimeBreakdownAccumulator{}
	errorEndpoints := map[string]*usageOverviewRealtimeBreakdownAccumulator{}
	recentErrors := make([]dto.RealtimeErrorRecord, 0, 20)
	var visibleRequests int64
	var visibleFailures int64
	var unknownErrorDetails int64
	...
	for _, realtimeEvent := range events {
		...
		timestamp := timeutil.NormalizeStorageTime(event.Timestamp)
		applyUsageOverviewRealtimeConcurrency(buckets, event, readStart, span, end)
		...
		if visibleEvent {
			...
			if event.Failed {
				visibleFailures++
				if event.StatusCode == 0 && strings.TrimSpace(event.ErrorType) == "" && strings.TrimSpace(event.ErrorMessage) == "" {
					unknownErrorDetails++
				}
				applyUsageOverviewRealtimeError(
					realtimeEvent,
					identityLookup,
					errorStatusCodes,
					errorTypes,
	...
	return finalizeUsageOverviewRealtime(
		window,
		span,
		start,
		end,
...
func applyUsageOverviewRealtimeConcurrency(buckets []usageOverviewRealtimeBucket, event entities.UsageEvent, start time.Time, span time.Duration, windowEnd time.Time) {
	if event.LatencyMS <= 0 || span <= 0 || len(buckets) == 0 {
		return
	}
	requestStart := timeutil.NormalizeStorageTime(event.Timestamp)
	requestEnd := requestStart.Add(time.Duration(event.LatencyMS) * time.Millisecond)
	windowStart := timeutil.NormalizeStorageTime(start)
	windowEnd = timeutil.NormalizeStorageTime(windowEnd)
	if !requestEnd.After(windowStart) || !requestStart.Before(windowEnd) {
		return
	}
	for index := range buckets {
		bucketStart := buckets[index].bucketStart
		bucketEnd := bucketStart.Add(span)
		...
		buckets[index].concurrencyMS += overlapEnd.Sub(overlapStart).Milliseconds()
		buckets[index].intervals = append(buckets[index].intervals, usageOverviewRealtimeInterval{start: overlapStart, end: overlapEnd})
	}
}

func applyUsageOverviewRealtimeError(
	realtimeEvent usageOverviewRealtimeEvent,
	identityLookup analysisIdentityLookup,
	statusCodes map[string]*usageOverviewRealtimeBreakdownAccumulator,
	errorTypes map[string]*usageOverviewRealtimeBreakdownAccumulator,
	models map[string]*usageOverviewRealtimeBreakdownAccumulator,
	authFiles map[string]*usageOverviewRealtimeBreakdownAccumulator,
	endpoints map[string]*usageOverviewRealtimeBreakdownAccumulator,
	recentErrors *[]dto.RealtimeErrorRecord,
) {
	event := realtimeEvent.event
	statusKey := "unknown"
	statusLabel := "Unknown"
	if event.StatusCode > 0 {
		...
		statusLabel = "HTTP " + statusKey
	}
	addUsageOverviewRealtimeBreakdown(statusCodes, statusKey, statusLabel)
	errorType := strings.TrimSpace(event.ErrorType)
	if errorType == "" {
		errorType = "unknown_error"
	}
	addUsageOverviewRealtimeBreakdown(errorTypes, errorType, errorType)
	model := normalizeUsageOverviewDimension(event.Model)
	addUsageOverviewRealtimeBreakdown(models, model, model)
	endpoint := strings.TrimSpace(event.Endpoint)
	if endpoint == "" {
		endpoint = "unknown"
	}
	addUsageOverviewRealtimeBreakdown(endpoints, endpoint, endpoint)

	authIndex := strings.TrimSpace(event.AuthIndex)
	authLabel := authIndex
	authFile, aiProvider := usageOverviewRealtimeIdentityTargets(realtimeEvent, identityLookup)
	if authFile != nil {
		authIndex = authFile.identity
		authLabel = authFile.label
	} else if aiProvider != nil {
		authIndex = aiProvider.identity
		authLabel = aiProvider.label
	}
	if authIndex == "" {
		authIndex = "unknown"
	}
	...
	addUsageOverviewRealtimeBreakdown(authFiles, authIndex, authLabel)
	*recentErrors = append(*recentErrors, dto.RealtimeErrorRecord{
		Timestamp:  timeutil.NormalizeStorageTime(event.Timestamp),
		RequestID:  strings.TrimSpace(event.RequestID),
		StatusCode: event.StatusCode,
		ErrorType:  errorType,
...
func addUsageOverviewRealtimeBreakdown(totals map[string]*usageOverviewRealtimeBreakdownAccumulator, key, label string) {
	if item, ok := totals[key]; ok {
		item.count++
		return
	}
	totals[key] = &usageOverviewRealtimeBreakdownAccumulator{key: key, label: label, count: 1}
}

func usageOverviewRealtimePeakConcurrency(intervals []usageOverviewRealtimeInterval) int64 {
	type change struct {
		at    time.Time
		delta int64
	}
	changes := make([]change, 0, len(intervals)*2)
	for _, interval := range intervals {
		if !interval.end.After(interval.start) {
			continue
		}
		changes = append(changes, change{at: interval.start, delta: 1}, change{at: interval.end, delta: -1})
	}
...
func finalizeUsageOverviewRealtimeBreakdown(totals map[string]*usageOverviewRealtimeBreakdownAccumulator, totalFailures int64) []dto.RealtimeBreakdownItemRecord {
	rows := make([]dto.RealtimeBreakdownItemRecord, 0, len(totals))
	for _, item := range totals {
		share := 0.0
		if totalFailures > 0 {
			share = float64(item.count) / float64(totalFailures) * 100
		}
		rows = append(rows, dto.RealtimeBreakdownItemRecord{Key: item.key, Label: item.label, Count: item.count, Share: share})
	}
...
func finalizeUsageOverviewRealtimeErrors(errors []dto.RealtimeErrorRecord) []dto.RealtimeErrorRecord {
	sort.SliceStable(errors, func(i, j int) bool { return errors[i].Timestamp.After(errors[j].Timestamp) })
	if len(errors) > 20 {
		errors = errors[:20]
	}
	return errors
...
func aggregateUsageOverviewRealtimeBucket(buckets []usageOverviewRealtimeBucket, index, bucketCount int) usageOverviewRealtimeBucket {
	...
	for bucketIndex := start; bucketIndex <= index; bucketIndex++ {
		...
		aggregated.failures += bucket.failures
		...
		aggregated.concurrencyMS += bucket.concurrencyMS
		aggregated.intervals = append(aggregated.intervals, bucket.intervals...)
	}
	return aggregated
}

func finalizeUsageOverviewRealtime(
	window, span time.Duration,
	windowStart, windowEnd time.Time,
	buckets []usageOverviewRealtimeBucket,
	visibleStartIndex int,
	modelUsage, apiKeyUsage, authFileUsage, aiProviderUsage map[string]*usageOverviewRealtimeTopAccumulator,
	visibleRequests, visibleFailures, unknownErrorDetails int64,
	errorStatusCodes, errorTypes, errorModels, errorAuthFiles, errorEndpoints map[string]*usageOverviewRealtimeBreakdownAccumulator,
	recentErrors []dto.RealtimeErrorRecord,
) dto.UsageOverviewRealtimeRecord {
	...
	concurrencyLevel := make([]dto.RealtimeConcurrencyPointRecord, 0, visibleBucketCount)
	errorLevel := make([]dto.RealtimeErrorLevelPointRecord, 0, visibleBucketCount)
	aggregationWindow := usageOverviewRealtimeAggregationWindow(window)
	aggregationBucketCount := usageOverviewRealtimeAggregationBucketCount(span, aggregationWindow)
	...
	for index := visibleStartIndex; index < len(buckets); index++ {
		...
		if aggregationWindow > 0 {
			averageConcurrency = float64(rollingBucket.concurrencyMS) / float64(aggregationWindow.Milliseconds())
		}
		concurrencyLevel = append(concurrencyLevel, dto.RealtimeConcurrencyPointRecord{
			Bucket:             bucketKey,
			AverageConcurrency: averageConcurrency,
			PeakConcurrency:    usageOverviewRealtimePeakConcurrency(rollingBucket.intervals),
		})
		failureRate := 0.0
		if rollingBucket.requests > 0 {
			failureRate = float64(rollingBucket.failures) / float64(rollingBucket.requests) * 100
		}
		errorLevel = append(errorLevel, dto.RealtimeErrorLevelPointRecord{
			Bucket:      bucketKey,
			Failures:    rollingBucket.failures,
	...
	if visibleRequests > 0 {
		failureRate = float64(visibleFailures) / float64(visibleRequests) * 100
	}
	return dto.UsageOverviewRealtimeRecord{
		...
		Errors: dto.RealtimeErrorSummaryRecord{
			TotalFailures:  visibleFailures,
			FailureRate:    failureRate,
			UnknownDetails: unknownErrorDetails,
			StatusCodes:    finalizeUsageOverviewRealtimeBreakdown(errorStatusCodes, visibleFailures),
			Types:          finalizeUsageOverviewRealtimeBreakdown(errorTypes, visibleFailures),
			Models:         finalizeUsageOverviewRealtimeBreakdown(errorModels, visibleFailures),
			AuthFiles:      finalizeUsageOverviewRealtimeBreakdown(errorAuthFiles, visibleFailures),
			Endpoints:      finalizeUsageOverviewRealtimeBreakdown(errorEndpoints, visibleFailures),
			RecentErrors:   finalizeUsageOverviewRealtimeErrors(recentErrors),
		},
	}
}
