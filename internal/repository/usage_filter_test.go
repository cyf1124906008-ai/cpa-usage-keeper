...
func TestBuildUsageOverviewIncludesEarliestAvailableEvent(t *testing.T) {
	withRepositoryTestLocation(t, "Asia/Shanghai")
	db, err := OpenDatabase(config.Config{SQLitePath: filepath.Join(t.TempDir(), "usage-overview-data-start.db")})
	if err != nil {
		t.Fatalf("OpenDatabase returned error: %v", err)
	}
	closeTestDatabase(t, db)

	earliest := time.Date(2026, 7, 7, 21, 54, 28, 657000000, time.Local)
	later := earliest.Add(15 * time.Minute)
	if _, _, err := InsertUsageEvents(db, []entities.UsageEvent{
		{EventKey: "data-start", APIGroupKey: "provider-a", Model: "gpt-test", Timestamp: earliest, InputTokens: 10, TotalTokens: 10},
		{EventKey: "data-later", APIGroupKey: "provider-a", Model: "gpt-test", Timestamp: later, InputTokens: 20, TotalTokens: 20},
	}); err != nil {
		t.Fatalf("InsertUsageEvents returned error: %v", err)
	}

	dataAvailableFrom, err := FindUsageDataAvailableFrom(db, "")
	if err != nil {
		t.Fatalf("FindUsageDataAvailableFrom returned error: %v", err)
	}
	if dataAvailableFrom == nil || !dataAvailableFrom.Equal(earliest) {
		t.Fatalf("expected data availability to start at %s, got %v", earliest, dataAvailableFrom)
	}
}
