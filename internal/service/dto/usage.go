...
type UsageOverviewRealtime struct {
	...
	ConcurrencyLevel        []RealtimeConcurrencyPoint
	ErrorLevel              []RealtimeErrorLevelPoint
	Errors                  RealtimeErrorSummary
	CurrentConcurrency      int64
	GatewayPeakConcurrency  int64
	RuntimeMetricsAvailable bool
}

type RealtimeConcurrencyPoint struct {
	Bucket             string
	AverageConcurrency float64
	PeakConcurrency    int64
}

type RealtimeErrorLevelPoint struct {
	Bucket      string
	Failures    int64
	FailureRate float64
}

type RealtimeBreakdownItem struct {
	Key   string
	Label string
	Count int64
	Share float64
...
type RealtimeErrorSummary struct {
	TotalFailures  int64
	FailureRate    float64
	UnknownDetails int64
	StatusCodes    []RealtimeBreakdownItem
	Types          []RealtimeBreakdownItem
	Models         []RealtimeBreakdownItem
	AuthFiles      []RealtimeBreakdownItem
