...
func NewUsageServiceWithRecentCache(db *gorm.DB, recentUsage *repository.UsageRecentEventCache) UsageProvider {
	return NewUsageServiceWithOptions(db, UsageServiceOptions{RecentUsage: recentUsage})
}
...
func NewUsageServiceWithOptions(db *gorm.DB, options UsageServiceOptions) UsageProvider {
	...
	return &usageService{
		...
		runtimeMetricsURL: strings.TrimSpace(options.RuntimeMetricsURL),
		httpClient:        client,
	}
}
...
func (s *usageService) GetUsageOverview(ctx context.Context, filter servicedto.UsageFilter) (*servicedto.UsageOverviewSnapshot, error) {
	...
	dataAvailableFrom, err := repository.FindUsageDataAvailableFrom(s.db.WithContext(ctx), apiGroupKey)
	if err != nil {
		return nil, err
	}
...
func (s *usageService) GetUsageOverviewRealtime(ctx context.Context, filter servicedto.UsageFilter) (*servicedto.UsageOverviewRealtime, error) {
	...
	if metrics, ok := s.loadRuntimeMetrics(ctx); ok {
		result.CurrentConcurrency = metrics.CurrentInflight
		result.GatewayPeakConcurrency = metrics.PeakInflight
		result.RuntimeMetricsAvailable = true
	}
	return &result, nil
}
...
func mapUsageOverviewRealtime(realtime repodto.UsageOverviewRealtimeRecord) servicedto.UsageOverviewRealtime {
	return servicedto.UsageOverviewRealtime{
		...
		ConcurrencyLevel:     mapRealtimeConcurrencyLevel(realtime.ConcurrencyLevel),
		ErrorLevel:           mapRealtimeErrorLevel(realtime.ErrorLevel),
		Errors:               mapRealtimeErrors(realtime.Errors),
	}
}

func mapRealtimeConcurrencyLevel(points []repodto.RealtimeConcurrencyPointRecord) []servicedto.RealtimeConcurrencyPoint {
	result := make([]servicedto.RealtimeConcurrencyPoint, 0, len(points))
	for _, point := range points {
		result = append(result, servicedto.RealtimeConcurrencyPoint{
...
func mapRealtimeErrorLevel(points []repodto.RealtimeErrorLevelPointRecord) []servicedto.RealtimeErrorLevelPoint {
	result := make([]servicedto.RealtimeErrorLevelPoint, 0, len(points))
	for _, point := range points {
		result = append(result, servicedto.RealtimeErrorLevelPoint{Bucket: point.Bucket, Failures: point.Failures, FailureRate: point.FailureRate})
	}
	return result
}

func mapRealtimeBreakdown(items []repodto.RealtimeBreakdownItemRecord) []servicedto.RealtimeBreakdownItem {
	result := make([]servicedto.RealtimeBreakdownItem, 0, len(items))
	for _, item := range items {
		result = append(result, servicedto.RealtimeBreakdownItem{Key: item.Key, Label: item.Label, Count: item.Count, Share: item.Share})
	}
	return result
}

func mapRealtimeErrors(errors repodto.RealtimeErrorSummaryRecord) servicedto.RealtimeErrorSummary {
	recent := make([]servicedto.RealtimeErrorRecord, 0, len(errors.RecentErrors))
	for _, item := range errors.RecentErrors {
		recent = append(recent, servicedto.RealtimeErrorRecord{
	...
	return servicedto.RealtimeErrorSummary{
		TotalFailures: errors.TotalFailures, FailureRate: errors.FailureRate, UnknownDetails: errors.UnknownDetails,
		StatusCodes: mapRealtimeBreakdown(errors.StatusCodes), Types: mapRealtimeBreakdown(errors.Types), Models: mapRealtimeBreakdown(errors.Models),
		AuthFiles: mapRealtimeBreakdown(errors.AuthFiles), Endpoints: mapRealtimeBreakdown(errors.Endpoints), RecentErrors: recent,
	}
}
