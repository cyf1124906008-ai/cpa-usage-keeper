...
type usageOverviewResponse struct {
	...
	DataAvailableFrom *time.Time                 `json:"data_available_from,omitempty"`
}
...
type usageOverviewRealtime struct {
	...
	ConcurrencyLevel        []usageOverviewConcurrencyPoint   `json:"concurrency_level"`
	ErrorLevel              []usageOverviewErrorLevelPoint    `json:"error_level"`
	Errors                  usageOverviewErrorSummary         `json:"errors"`
	CurrentConcurrency      int64                             `json:"current_concurrency"`
	GatewayPeakConcurrency  int64                             `json:"gateway_peak_concurrency"`
...
type usageOverviewConcurrencyPoint struct {
	Bucket             string  `json:"bucket"`
	AverageConcurrency float64 `json:"average_concurrency"`
	PeakConcurrency    int64   `json:"peak_concurrency"`
}

type usageOverviewErrorLevelPoint struct {
	Bucket      string  `json:"bucket"`
	Failures    int64   `json:"failures"`
	FailureRate float64 `json:"failure_rate"`
}

type usageOverviewBreakdownItem struct {
	Key   string  `json:"key"`
	Label string  `json:"label"`
	Count int64   `json:"count"`
	Share float64 `json:"share"`
...
type usageOverviewRecentError struct {
...
type usageOverviewErrorSummary struct {
	TotalFailures  int64                        `json:"total_failures"`
	FailureRate    float64                      `json:"failure_rate"`
	UnknownDetails int64                        `json:"unknown_details"`
	StatusCodes    []usageOverviewBreakdownItem `json:"status_codes"`
	Types          []usageOverviewBreakdownItem `json:"types"`
	Models         []usageOverviewBreakdownItem `json:"models"`
	AuthFiles      []usageOverviewBreakdownItem `json:"auth_files"`
	Endpoints      []usageOverviewBreakdownItem `json:"endpoints"`
	RecentErrors   []usageOverviewRecentError   `json:"recent_errors"`
}
...
func writeUsageOverviewResponse(c *gin.Context, usageProvider service.UsageProvider, filter servicedto.UsageFilter) {
	...
	var dataAvailableFrom *time.Time
	if overview != nil {
		usage = overview.Usage
		dataAvailableFrom = overview.DataAvailableFrom
	}
	c.JSON(http.StatusOK, usageOverviewResponse{
...
func emptyUsageOverviewRealtime(window string) usageOverviewRealtime {
	...
	return usageOverviewRealtime{
		...
		ConcurrencyLevel: []usageOverviewConcurrencyPoint{},
		ErrorLevel:       []usageOverviewErrorLevelPoint{},
		Errors: usageOverviewErrorSummary{
			StatusCodes: []usageOverviewBreakdownItem{}, Types: []usageOverviewBreakdownItem{}, Models: []usageOverviewBreakdownItem{},
			AuthFiles: []usageOverviewBreakdownItem{}, Endpoints: []usageOverviewBreakdownItem{}, RecentErrors: []usageOverviewRecentError{},
		},
	}
}
...
func buildUsageOverviewRealtime(realtime *servicedto.UsageOverviewRealtime, window string, apiKeyInfos map[string]analysisAPIKeyInfo) usageOverviewRealtime {
	...
	result := usageOverviewRealtime{
		...
		ConcurrencyLevel:        make([]usageOverviewConcurrencyPoint, 0, len(realtime.ConcurrencyLevel)),
		ErrorLevel:              make([]usageOverviewErrorLevelPoint, 0, len(realtime.ErrorLevel)),
		Errors:                  mapUsageOverviewErrors(realtime.Errors),
		CurrentConcurrency:      realtime.CurrentConcurrency,
		GatewayPeakConcurrency:  realtime.GatewayPeakConcurrency,
		RuntimeMetricsAvailable: realtime.RuntimeMetricsAvailable,
	}
	if result.Window == "" {
	...
	for _, point := range realtime.ConcurrencyLevel {
		result.ConcurrencyLevel = append(result.ConcurrencyLevel, usageOverviewConcurrencyPoint{
			Bucket: point.Bucket, AverageConcurrency: point.AverageConcurrency, PeakConcurrency: point.PeakConcurrency,
		})
	}
	for _, point := range realtime.ErrorLevel {
		result.ErrorLevel = append(result.ErrorLevel, usageOverviewErrorLevelPoint{
			Bucket: point.Bucket, Failures: point.Failures, FailureRate: point.FailureRate,
		})
	}
	return result
}

func mapUsageOverviewBreakdown(items []servicedto.RealtimeBreakdownItem) []usageOverviewBreakdownItem {
	result := make([]usageOverviewBreakdownItem, 0, len(items))
	for _, item := range items {
		result = append(result, usageOverviewBreakdownItem{Key: item.Key, Label: item.Label, Count: item.Count, Share: item.Share})
	}
	return result
}

func mapUsageOverviewErrors(errors servicedto.RealtimeErrorSummary) usageOverviewErrorSummary {
	recent := make([]usageOverviewRecentError, 0, len(errors.RecentErrors))
	for _, item := range errors.RecentErrors {
		recent = append(recent, usageOverviewRecentError{
			Timestamp: item.Timestamp, RequestID: item.RequestID, StatusCode: item.StatusCode, ErrorType: item.ErrorType,
			Message: item.Message, Model: item.Model, AuthIndex: item.AuthIndex, AuthLabel: item.AuthLabel, Endpoint: item.Endpoint,
		})
	}
	return usageOverviewErrorSummary{
		TotalFailures: errors.TotalFailures, FailureRate: errors.FailureRate, UnknownDetails: errors.UnknownDetails,
		StatusCodes: mapUsageOverviewBreakdown(errors.StatusCodes), Types: mapUsageOverviewBreakdown(errors.Types), Models: mapUsageOverviewBreakdown(errors.Models),
		AuthFiles: mapUsageOverviewBreakdown(errors.AuthFiles), Endpoints: mapUsageOverviewBreakdown(errors.Endpoints), RecentErrors: recent,
	}
}
