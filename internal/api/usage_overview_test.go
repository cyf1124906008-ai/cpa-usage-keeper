...
func TestUsageOverviewResponseIncludesResolvedRangeAndTimezone(t *testing.T) {
	...
	dataAvailableFrom := time.Date(2026, 7, 7, 21, 54, 28, 0, location)
	provider := &usageFilterStub{overview: &servicedto.UsageOverviewSnapshot{DataAvailableFrom: &dataAvailableFrom}}
	...
	if !contains(body, `"timezone":"Asia/Shanghai"`) || !contains(body, `"range_start":"`+expectedStart+`"`) || !contains(body, `"range_end":"`+expectedEnd+`"`) || !contains(body, `"data_available_from":"`+dataAvailableFrom.Format(time.RFC3339Nano)+`"`) {
...
func assertUsageOverviewResponseShape(t *testing.T, body string) {
	...
	assertAllowedJSONKeys(t, decoded, "overview response", body, "usage", "summary", "series", "service_health", "timezone", "range_start", "range_end", "data_available_from")
