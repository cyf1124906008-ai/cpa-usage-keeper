...
function normalizeOverviewRealtimeBlock(
  ...
  return {
    ...
    concurrency_level: block.concurrency_level ?? [],
    error_level: block.error_level ?? [],
    errors: {
      total_failures: block.errors?.total_failures ?? 0,
      failure_rate: block.errors?.failure_rate ?? 0,
      unknown_details: block.errors?.unknown_details ?? 0,
      status_codes: block.errors?.status_codes ?? [],
      types: block.errors?.types ?? [],
      models: block.errors?.models ?? [],
      auth_files: block.errors?.auth_files ?? [],
      endpoints: block.errors?.endpoints ?? [],
      recent_errors: block.errors?.recent_errors ?? [],
    },
    current_concurrency: block.current_concurrency ?? 0,
    gateway_peak_concurrency: block.gateway_peak_concurrency ?? 0,
    runtime_metrics_available: block.runtime_metrics_available ?? false,
  }
}
