...
describe('UsagePage toolbar styles', () => {
  ...
  it('renders the realtime overview panel below Request Health Timeline with the planned responsive grid', () => {
    const pinnedRatesIndex = usagePageSource.indexOf('<OverviewRealtimePinnedRates')
    const dailyAverageIndex = usagePageSource.indexOf('<DailyAveragePanel usage={dailyAveragePanelUsage} loading={overviewDisplayLoading} reserveVisible={reserveDailyAveragePanel} />')
    expect(pinnedRatesIndex).toBeGreaterThanOrEqual(0)
    expect(pinnedRatesIndex).toBeLessThan(dailyAverageIndex)
    expect(usagePageStyles).toMatch(/\.overviewRealtimePinnedRates\s*\{[\s\S]*?grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);/)
    expect(usagePageStyles).toMatch(/\.overviewRealtimePinnedRates\s*\{[\s\S]*?@include mobile\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\);/)
