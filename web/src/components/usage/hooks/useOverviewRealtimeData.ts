...
import { USAGE_STATS_REALTIME_STALE_TIME_MS, useUsageStatsStore } from '@/stores';
...
export function useOverviewRealtimeData(options: UseOverviewRealtimeDataOptions = {}): UseOverviewRealtimeDataReturn {
  ...
  const loadRealtime = useCallback(async () => {
    try {
      await loadUsageStatsRealtime({
        force: true,
        staleTimeMs: USAGE_STATS_REALTIME_STALE_TIME_MS,
        apiKeyId,
        realtimeWindow,
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    }, USAGE_STATS_REALTIME_STALE_TIME_MS);
