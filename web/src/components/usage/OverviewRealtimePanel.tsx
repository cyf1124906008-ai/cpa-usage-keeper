...
type ErrorDimensionKey = 'status_codes' | 'types' | 'models' | 'auth_files' | 'endpoints';
...
interface OverviewRealtimePinnedRatesProps {
  realtime?: OverviewRealtimeBlock;
  loading: boolean;
}
...
function RealtimeKPI({ label, value, detail, tone = 'default' }: { label: string; value: string; detail: string; tone?: 'default' | 'danger' | 'success' }) {
  return (
    <div className={`${styles.overviewRealtimeKpi} ${tone === 'danger' ? styles.overviewRealtimeKpiDanger : tone === 'success' ? styles.overviewRealtimeKpiSuccess : ''}`.trim()}>
      <span className={styles.overviewRealtimeKpiLabel}>{label}</span>
      <strong className={styles.overviewRealtimeKpiValue}>{value}</strong>
      <span className={styles.overviewRealtimeKpiDetail}>{detail}</span>
    </div>
  );
}

function ErrorBreakdownList({ items, emptyLabel }: { items: RealtimeBreakdownItem[]; emptyLabel: string }) {
  if (items.length === 0) return <div className={styles.overviewRealtimeErrorEmpty}>{emptyLabel}</div>;
  return (
    <div className={styles.overviewRealtimeErrorBreakdownList}>
      {items.map((item) => (
        <div key={item.key} className={styles.overviewRealtimeErrorBreakdownItem}>
          <div className={styles.overviewRealtimeErrorBreakdownTopline}>
            <span title={item.label}>{item.label}</span>
            <strong>{item.count.toLocaleString()} · {formatFixedTwoDecimals(safeNumber(item.share))}%</strong>
          </div>
          <div className={styles.overviewRealtimeErrorTrack}>
            <span style={{ width: `${Math.max(0, Math.min(100, safeNumber(item.share)))}%` }} />
          </div>
        </div>
      ))}
    </div>
...
function formatRealtimeErrorTime(value: string, timezone?: string): string {
  const parsed = Date.parse(value);
  if (!Number.isFinite(parsed)) return value;
  try {
    return new Intl.DateTimeFormat('zh-CN', {
      month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit',
      hourCycle: 'h23', timeZone: getIntlTimeZone(timezone),
...
export function OverviewRealtimePinnedRates({ realtime, loading }: OverviewRealtimePinnedRatesProps) {
  const { t } = useTranslation();
  const latestRPM = realtime ? latestNumber(realtime.request_level.map((point) => safeNumber(point.requests_per_minute))) : null;
  const latestTPM = realtime ? latestNumber(realtime.token_velocity.map((point) => safeNumber(point.tokens_per_minute))) : null;

  return (
    <section
      className={styles.overviewRealtimePinnedRates}
      aria-label={t('usage_stats.overview_realtime_section_title')}
      aria-busy={loading}
      aria-live="polite"
    >
      <RealtimeKPI
        label={t('usage_stats.overview_realtime_pinned_rpm')}
        value={latestRPM === null ? '--' : formatPerMinuteValue(latestRPM)}
        detail={t('usage_stats.overview_realtime_kpi_rolling')}
        tone="success"
      />
      <RealtimeKPI
        label={t('usage_stats.overview_realtime_pinned_tpm')}
        value={latestTPM === null ? '--' : Math.round(latestTPM).toLocaleString('en-US')}
        detail={t('usage_stats.overview_realtime_kpi_rolling')}
      />
    </section>
  );
...
export function OverviewRealtimePanel({ realtime, loading, error, window, onWindowChange, isDark, isMobile, timezone, visibleDimensions = DEFAULT_VISIBLE_DIMENSIONS }: OverviewRealtimePanelProps) {
  ...
  const [activeErrorDimension, setActiveErrorDimension] = useState<ErrorDimensionKey>('status_codes');
  ...
  const concurrencyValues = useMemo(() => (data.concurrency_level ?? []).map((point) => safeNumber(point.average_concurrency)), [data.concurrency_level]);
  const concurrencyPeakValues = useMemo(() => (data.concurrency_level ?? []).map((point) => safeNumber(point.peak_concurrency)), [data.concurrency_level]);
  const errorRateValues = useMemo(() => (data.error_level ?? []).map((point) => safeNumber(point.failure_rate)), [data.error_level]);
  const errors = data.errors ?? emptyRealtime(window).errors!;
  ...
  const concurrencyEmptyLabel = !hasFiniteNumber(concurrencyValues) ? t('usage_stats.overview_realtime_concurrency_empty') : undefined;
  ...
  const concurrencyChartData = useMemo<ChartData<'line', Array<number | null>, string>>(() => ({
    labels,
    datasets: [
      {
        label: t('usage_stats.overview_realtime_concurrency_average'),
        data: concurrencyValues,
        borderColor: CHART_COLORS.concurrency,
        backgroundColor: `${CHART_COLORS.concurrency}20`,
        fill: true,
      },
      {
        label: t('usage_stats.overview_realtime_concurrency_peak'),
        data: concurrencyPeakValues,
        borderColor: CHART_COLORS.concurrencyPeak,
        backgroundColor: 'transparent',
        borderDash: [5, 4],
        fill: false,
      },
    ],
  }), [concurrencyPeakValues, concurrencyValues, labels, t]);
  const errorChartData = useMemo(() => buildSingleLineData(labels, t('usage_stats.overview_realtime_error_rate'), errorRateValues, CHART_COLORS.error), [errorRateValues, labels, t]);
  ...
  const errorDimensions = useMemo<ErrorDimension[]>(() => [
    { key: 'status_codes', labelKey: 'usage_stats.overview_realtime_errors_status', items: errors.status_codes },
    { key: 'types', labelKey: 'usage_stats.overview_realtime_errors_type', items: errors.types },
    { key: 'models', labelKey: 'usage_stats.overview_realtime_errors_model', items: errors.models },
    { key: 'auth_files', labelKey: 'usage_stats.overview_realtime_errors_auth', items: errors.auth_files },
    { key: 'endpoints', labelKey: 'usage_stats.overview_realtime_errors_endpoint', items: errors.endpoints },
  ], [errors.auth_files, errors.endpoints, errors.models, errors.status_codes, errors.types]);
  const visibleErrorDimension = errorDimensions.find((dimension) => dimension.key === activeErrorDimension) ?? errorDimensions[0];
  const latestRPM = latestNumber(requestValues);
  const latestTPM = latestNumber(tokenValues);
  const latestAverageConcurrency = latestNumber(concurrencyValues);
  const windowPeakConcurrency = concurrencyPeakValues.length > 0 ? Math.max(...concurrencyPeakValues) : 0;
    ...
    <div className={styles.overviewRealtimeSection}>
      ...
      {showErrorOnly ? (
        ...
        <>
          ...
          <div className={styles.overviewRealtimeKpiGrid} aria-busy={loading}>
            <RealtimeKPI
              label={t('usage_stats.overview_realtime_rpm')}
              value={latestRPM === null ? '--' : formatPerMinuteValue(latestRPM)}
              detail={t('usage_stats.overview_realtime_kpi_rolling')}
            />
            <RealtimeKPI
              label={t('usage_stats.overview_realtime_tpm')}
              value={latestTPM === null ? '--' : Math.round(latestTPM).toLocaleString('en-US')}
              detail={t('usage_stats.overview_realtime_kpi_rolling')}
            />
            <RealtimeKPI
              label={t('usage_stats.overview_realtime_concurrency_current')}
              value={data.runtime_metrics_available ? formatCompactNumber(safeNumber(data.current_concurrency)) : '--'}
              detail={data.runtime_metrics_available ? t('usage_stats.overview_realtime_kpi_gateway_live') : t('usage_stats.overview_realtime_kpi_gateway_unavailable')}
              tone={data.runtime_metrics_available ? 'success' : 'default'}
            />
            <RealtimeKPI
              label={t('usage_stats.overview_realtime_concurrency_peak')}
              value={formatCompactNumber(windowPeakConcurrency)}
              detail={latestAverageConcurrency === null ? t('usage_stats.overview_realtime_kpi_window_peak') : `${t('usage_stats.overview_realtime_concurrency_average')} ${formatFixedTwoDecimals(latestAverageConcurrency)}`}
            />
            <RealtimeKPI
              label={t('usage_stats.overview_realtime_error_rate')}
              value={`${formatFixedTwoDecimals(safeNumber(errors.failure_rate))}%`}
              detail={`${errors.total_failures.toLocaleString()} ${t('usage_stats.overview_realtime_errors_count_suffix')}`}
              tone={errors.total_failures > 0 ? 'danger' : 'success'}
            />
          </div>
          ...
          <RealtimeCard
            title={t('usage_stats.overview_realtime_concurrency')}
            metrics={[
              { label: t('usage_stats.overview_realtime_concurrency_current'), value: data.runtime_metrics_available ? formatCompactNumber(safeNumber(data.current_concurrency)) : '--' },
              { label: t('usage_stats.overview_realtime_concurrency_average'), value: latestAverageConcurrency === null ? '--' : formatFixedTwoDecimals(latestAverageConcurrency) },
              { label: t('usage_stats.overview_realtime_concurrency_peak'), value: formatCompactNumber(windowPeakConcurrency) },
            ]}
            metricsTooltip={t('usage_stats.overview_realtime_concurrency_hint')}
            full
          >
            <RealtimeChartFrame loading={loading} emptyLabel={concurrencyEmptyLabel}>
              <Line data={concurrencyChartData} options={lineOptions} />
            </RealtimeChartFrame>
          </RealtimeCard>
          ...
          <section className={`${styles.overviewRealtimeCard} ${styles.overviewRealtimeCardFull} ${styles.overviewRealtimeErrorCard}`}>
            <div className={styles.overviewRealtimeCardHeader}>
              <div>
                <h3 className={styles.overviewRealtimeCardTitle}>{t('usage_stats.overview_realtime_errors_title')}</h3>
                <p className={styles.overviewRealtimeErrorSubtitle}>
                  {errors.total_failures.toLocaleString()} {t('usage_stats.overview_realtime_errors_count_suffix')} · {formatFixedTwoDecimals(safeNumber(errors.failure_rate))}%
                  {errors.unknown_details > 0 ? ` · ${errors.unknown_details.toLocaleString()} ${t('usage_stats.overview_realtime_errors_historical_unknown')}` : ''}
                </p>
              </div>
              <div className={styles.overviewRealtimeMetrics}>
                <span className={`${styles.overviewRealtimeMetric} ${errors.total_failures > 0 ? styles.overviewRealtimeMetricDown : styles.overviewRealtimeMetricUp}`.trim()}>
                  <span className={styles.overviewRealtimeMetricLabel}>{t('usage_stats.overview_realtime_error_rate')}</span>
                  <span className={styles.overviewRealtimeMetricValue}>{formatFixedTwoDecimals(safeNumber(errors.failure_rate))}%</span>
                </span>
              </div>
            </div>
            <div className={styles.overviewRealtimeErrorLayout}>
              <div className={styles.overviewRealtimeErrorTrend}>
                <h4>{t('usage_stats.overview_realtime_errors_trend')}</h4>
                <RealtimeChartFrame loading={loading} emptyLabel={errors.total_failures === 0 ? t('usage_stats.overview_realtime_errors_empty') : undefined}>
                  <Line data={errorChartData} options={percentLineOptions} />
                </RealtimeChartFrame>
              </div>
              <div className={styles.overviewRealtimeErrorBreakdown}>
                <div className={styles.overviewRealtimeDimensionTabs} role="group" aria-label={t('usage_stats.overview_realtime_errors_breakdown')}>
                  {errorDimensions.map((dimension) => (
                    <button
                      key={dimension.key}
                      type="button"
                      className={`${styles.overviewRealtimeDimensionTab} ${visibleErrorDimension.key === dimension.key ? styles.overviewRealtimeDimensionTabActive : ''}`.trim()}
                      onClick={() => setActiveErrorDimension(dimension.key)}
                      aria-pressed={visibleErrorDimension.key === dimension.key}
                    >
                      {t(dimension.labelKey)}
                    </button>
                  ))}
                </div>
                <ErrorBreakdownList items={visibleErrorDimension.items} emptyLabel={t('usage_stats.overview_realtime_errors_empty')} />
              </div>
            </div>
            <div className={styles.overviewRealtimeRecentErrors}>
              <h4>{t('usage_stats.overview_realtime_errors_recent')}</h4>
              {errors.recent_errors.length === 0 ? (
                <div className={styles.overviewRealtimeErrorEmpty}>{t('usage_stats.overview_realtime_errors_empty')}</div>
              ) : (
                <div className={styles.overviewRealtimeErrorTableScroller}>
                  <table className={styles.overviewRealtimeErrorTable}>
                    <thead>
                      <tr>
                        <th>{t('usage_stats.overview_realtime_errors_time')}</th>
                        <th>{t('usage_stats.overview_realtime_errors_status')}</th>
                        <th>{t('usage_stats.overview_realtime_errors_type')}</th>
                        <th>{t('usage_stats.overview_realtime_errors_model')}</th>
                        <th>{t('usage_stats.overview_realtime_errors_auth')}</th>
                        <th>{t('usage_stats.overview_realtime_errors_message')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {errors.recent_errors.slice(0, 12).map((item, index) => (
                        <tr key={`${item.request_id ?? item.timestamp}-${index}`}>
                          <td>{formatRealtimeErrorTime(item.timestamp, data.timezone ?? timezone)}</td>
                          <td><span className={styles.overviewRealtimeErrorStatus}>{item.status_code > 0 ? item.status_code : '--'}</span></td>
                          <td>{item.error_type || 'unknown_error'}</td>
                          <td title={item.model}>{item.model}</td>
                          <td title={item.auth_label ?? item.auth_index}>{item.auth_label ?? item.auth_index ?? '--'}</td>
                          <td title={item.message ?? ''}>{item.message || t('usage_stats.overview_realtime_errors_no_message')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
