...
import { OverviewRealtimePanel, OverviewRealtimePinnedRates } from './OverviewRealtimePanel';
...
const realtime: OverviewRealtimeBlock = {
  ...
  concurrency_level: [
    { bucket: '2026-06-09T11:55:00Z', average_concurrency: 3.5, peak_concurrency: 5 },
    { bucket: '2026-06-09T11:55:30Z', average_concurrency: 4.25, peak_concurrency: 7 },
  ],
  error_level: [
    { bucket: '2026-06-09T11:55:00Z', failures: 0, failure_rate: 0 },
    { bucket: '2026-06-09T11:55:30Z', failures: 1, failure_rate: 2.5 },
  ],
  errors: {
    total_failures: 1,
    failure_rate: 1.25,
    unknown_details: 0,
    status_codes: [{ key: '429', label: 'HTTP 429', count: 1, share: 100 }],
    types: [{ key: 'rate_limit', label: 'rate_limit', count: 1, share: 100 }],
    models: [{ key: 'gpt-5', label: 'gpt-5', count: 1, share: 100 }],
    auth_files: [{ key: 'auth-1', label: 'Claude Account', count: 1, share: 100 }],
    endpoints: [{ key: '/v1/responses', label: '/v1/responses', count: 1, share: 100 }],
    recent_errors: [{ timestamp: '2026-06-09T11:55:30Z', status_code: 429, error_type: 'rate_limit', message: 'rate limited', model: 'gpt-5', auth_label: 'Claude Account', endpoint: '/v1/responses' }],
  },
  current_concurrency: 6,
  gateway_peak_concurrency: 9,
...
describe('OverviewRealtimePanel', () => {
  it('renders live RPM and exact TPM in the pinned overview rates', () => {
    const html = renderToStaticMarkup(
      <OverviewRealtimePinnedRates
        realtime={{
          ...realtime,
          token_velocity: [
            ...realtime.token_velocity.slice(0, -1),
            { bucket: '2026-06-09T11:55:30Z', tokens_per_minute: 30_280_000, tokens: 15_140_000 },
          ],
        }}
        loading={false}
      />
    ...
    expect(html).toContain('usage_stats.overview_realtime_pinned_rpm');
    expect(html).toContain('usage_stats.overview_realtime_pinned_tpm');
    expect(html).toContain('30,280,000');
  });

  it('shows the realtime TPM KPI as a full number', () => {
    const html = renderToStaticMarkup(
  ...
  it('renders realtime chart layout with one full-width chart and two two-column rows', () => {
    ...
    expect(html).toContain('usage_stats.overview_realtime_concurrency');
    expect(html).toContain('usage_stats.overview_realtime_errors_title');
    expect(chartCapture.lineCalls).toHaveLength(5);
    ...
    expect(chartCapture.lineCalls[1].data.datasets[0].data).toEqual([3.5, 4.25]);
    expect(chartCapture.lineCalls[1].data.datasets[1].data).toEqual([5, 7]);
    expect(chartCapture.lineCalls[2].data.datasets[0].data).toEqual([2, 4]);
    expect(chartCapture.lineCalls[3].data.datasets[0].data).toEqual([25, 50]);
    expect(chartCapture.lineCalls[4].data.datasets[0].data).toEqual([0, 2.5]);
  });
  ...
  it('keeps gap spanning disabled for realtime charts', () => {
    ...
    expect(chartCapture.lineCalls[3].options.spanGaps).toBeUndefined();
    expect(chartCapture.lineCalls[4].options.spanGaps).toBeUndefined();
  ...
  it('keeps response axis logarithmic and cache axis light', () => {
    ...
    const cacheYAxis = chartCapture.lineCalls[3].options.scales?.y as { ticks?: { maxTicksLimit?: number } };
